...
tape('files', function(t) {
  ...
  base(function(db, blastDB) {
    db.put('fasta', {
      ...
      // wait for main database to build before continuing
      // to ensure that a separate update database is built
      setTimeout(function() {
        db.put('genbank', {
          seqFile: "test.gb",
          updated: 1
          ...
          // wait for update database to build before continuing
          // to ensure that the update database will have to be rebuilt
          setTimeout(function() {
            db.put('sbol', {
              seqFile: "test.sbol",
              updated: 1
